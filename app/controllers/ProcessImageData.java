 ...
 */
public class ProcessImageData extends Controller  {
  ...
  /**
   * Stores the type of entry - url or text or image.
   */
  public static String entryType = "image";
  /**
   ...
   * @param image the image entered by the user.
   */
  public static void processImage(File image, String imageName) {
      ...
      EntryDB.addImageEntry(entryType, keywords, keywordRelevance, imageToByte(image), imageName, ctx());
      keywords.clear();
    }
  /**
   * Converts the file image to a byte array.
   * @param imageToConvert File uploaded
   * @return byte array with image data.
   */
  private static byte[] imageToByte(File imageToConvert){
	  byte[] image = new byte[(int) imageToConvert.length()];
	    /* write the image data into the byte array */
	    InputStream inStream = null;
	    try {
	      inStream = new BufferedInputStream(new FileInputStream(imageToConvert));
	      inStream.read(image);
	    }
	    catch (IOException e) {
	      e.printStackTrace();
  ...
  private static void extractKeywords(File image) {
    try {
      ...
      String imagePostMode = "image=URI_ENCODED_DATA";
